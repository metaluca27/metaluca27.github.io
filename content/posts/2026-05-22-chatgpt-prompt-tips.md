...
요즘 많은 분들이 업무나 학습에 ChatGPT를 활용하고 계실 텐데요. 하지만 인공지능에게 질문을 던졌을 때 기대와 다른 뻔한 답변이나 겉도는 대답을 받아 실망한 경험이 한 번쯤은 있으실 겁니다. 저 역시 이 블로그의 글쓰기 파이프라인에 AI를 붙이는 과정에서 프롬프트를 수십 번 갈아엎었는데, 그 과정에서 "질문 한 줄 차이로 결과물이 이렇게까지 달라지나" 싶었던 순간이 많았습니다. 
...
특히 3번 부정 프롬프트는 제가 실제로 블로그 자동화 엔진에 넣어서 쓰고 있는 기법입니다. 처음에는 매번 "네, 작성해 드리겠습니다"로 시작하는 답변을 손으로 지우다가, 프롬프트에 "그런 어투를 완전히 배제하라"는 한 줄을 추가하니 후처리 작업이 통째로 사라졌습니다. 조건이 상세해질수록 답변을 필터링하고 다듬는 수고가 획기적으로 줄어듭니다.
